...
pub unsafe extern "system" fn dispatcher(_lp: *mut c_void) -> DWORD {
    ...
    loop {
        ...
        let syscall_name = std::str::from_utf8(&req.syscall_name)
            .unwrap_or_default()
            .trim_end_matches('\0');

        let ssn = match crate::internal::exports::SYSCALL_TABLE.get() {
            Some(table) => table.get(syscall_name).copied().unwrap_or_else(|| fatal_err("syscall not found")),
            None => fatal_err("syscall table not initialized"),
        };

        let stub = {
            let mut pool = G_STUB_POOL.lock().unwrap();
            let stub = pool.acquire().unwrap_or_else(|| fatal_err("no available stub"));
        
            // ensure it's RWX before modifying
            let mut old = 0;
            if winapi::um::memoryapi::VirtualProtect(stub as _, 32, PAGE_EXECUTE_READWRITE, &mut old) == 0 {
                fatal_err("VirtualProtect failed on stub");
            }
        
            stub
        };
        
        // now safely inject SSN
        *(stub.add(4) as *mut u32) = ssn;

        let fn_ptr: ABStubFn = transmute(stub);
        ...
        let ret = match req.arg_count {
            ...
            _ => fatal_err("invalid argument count"),
        };
