...
pub unsafe extern "C" fn ActiveBreach_launch() {
    ...
    print!("time to extract sycsallsslls");

    let syscall_table = internal::exports::extract_ssn(mapped_base, ab_handle_size);

    print!("i extracted syscalls");

    if let Err(e) = internal::exports::SYSCALL_TABLE.set(syscall_table) {
        println!("SYSCALL_TABLE.set() failed — double initialization?");
        std::process::exit(1);
    }

    print!("am going to zero and free");

    internal::file_buffer::zero_and_free(mapped_base, ab_handle_size);

    print!("hi lawl");

    internal::dispatch::G_AB_INITIALIZED_EVENT = CreateEventA(null_mut(), 1, 0, null());
    if internal::dispatch::G_AB_INITIALIZED_EVENT.is_null() {
        internal::err::fatal_err("failed to create initialization event");
    }
    ...
    WaitForSingleObject(internal::dispatch::G_AB_INITIALIZED_EVENT, INFINITE);
    CloseHandle(internal::dispatch::G_AB_INITIALIZED_EVENT);

    internal::dispatch::G_AB_INITIALIZED_EVENT = null_mut();
...
pub unsafe extern "C" fn ab_call(syscall_name: *const u8, args: *const usize, arg_count: usize) -> usize {
    ...
    let name = match CStr::from_ptr(syscall_name as *const c_char).to_str() {
        ...
        if !internal::exports::SYSCALL_TABLE
        .get()
        .expect("SYSCALL_TABLE not initialized")
        .contains_key(name)
    {
